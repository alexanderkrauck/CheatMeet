...
for (let attempt = 0; attempt < 20; attempt++) {
  try {
    ...
    if (!health.ok || data.status !== "ok" || !data.analysisConfigured || !data.transcriptionConfigured)
      throw new Error(
        "Server is unhealthy or Gemini/AssemblyAI configuration is missing.",
      );
    ...
    console.log(
      "Deployment health, Gemini/AssemblyAI configuration, SPA routes and PWA assets passed.",
    );
    process.exit(0);
  } catch (error) {
    lastError = error;
